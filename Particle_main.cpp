...
void macro() {
  ...
  TH1F *h2 = new TH1F("theta", "Theta", 200, 0, TMath::Pi());
  TH1F *h3 = new TH1F("phi", "phi", 200, 0, TMath::Pi() * 2);
  TH1F *pp = new TH1F("Impulse", "Impulse", 200, 0, 5);
  TH1F *trasvp = new TH1F("TraversP", "Traverse Impulse", 100, 0, 5);
  TH1F *energy =
      new TH1F("Energy", "Energy of particle", 100, 0, 5); // viene malissimo
  TH1F *massInvariant = new TH1F("MassInvariant", "Mass invariant", 80, 0, 2);
  TH1F *massID = new TH1F("massID", "Mass invariant discord", 80, 0, 2);
  TH1F *massIC = new TH1F("massIC", "Mass invariant concorde", 80, 0, 2);
  TH1F *mass_p_k_d =
    ...
    for (int j = 0; j < 100; ++j) {
      ...
      if (num_generate < 0.4) {
        ...
        if (num_generate < 0.995) {
          ...
          mass_inv_decad->Fill(
              Part[100 + counter].Mass_invariant(Part[100 + counter + 1]));
        } else {
