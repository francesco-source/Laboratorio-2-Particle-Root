...
Particle::Particle(const char *name, double x = 0,
		   double y = 0, double z = 0)
    : fPx{x}, fPy{y}, fPz{z}, fIParticle{FindParticle(name)} {}

Particle::Particle() : fPx{0}, fPy{0}, fPz{0},
fIParticle{-1} {}
...
void Particle::AddParticleType(const char *name,
		int charge, double mas double width) {
  if (FindParticle(name) == -1 && 
      fNParticleType < fMaxNumParticleType) {
    if (width == 0) {
      fParticleType[fNParticleType] = 
	      new PartycleType(charge, mass, name);
    } else {
      fParticleType[fNParticleType] =
          new ResonanceType(width, charge, mass, name);
    }
    ++fNParticleType;
    ...
    std::cout << "Metodo add-find Particle:Limite massimo di elementi già "
                 "raggiunto. \n";
  }
}
...
void Particle::print_particle() const {
  ...
  std::cout << " Nome della particella : "
            << fParticleType[fIParticle]->GetName() << " .\n";
...
double Particle::Mass_invariant(Particle &p) const {
  ...
  double momentum = ((fPx + p.Get_p_x()) * (fPx + p.Get_p_x())) +
                    ((fPy + p.Get_p_y()) * (fPy + p.Get_p_y())) +
                    ((fPz + p.Get_p_z()) * (fPz + p.Get_p_z()));
...
int Particle::Decay2body(Particle &dau1, Particle &dau2) const 
{
  ...
  if (massMot < massDau1 + massDau2) {
    printf("Decayment cannot be preformed because mass"
	   "is too low in this "
           "channel\n");
    return 2;
  }
  ...
  double pout =
      sqrt(
          (massMot * massMot - (massDau1 + massDau2)
	   * (massDau1 + massDau2)) *
          (massMot * massMot - (massDau1 - massDau2)
	   * (massDau1 - massDau2))) /
      massMot * 0.5;
  ...
  dau1.SetP(pout * sin(theta) * cos(phi), pout *
	    sin(theta) * sin(phi),
            pout * cos(theta));
  dau2.SetP(-pout * sin(theta) * cos(phi), -pout * 
	    sin(theta) * sin(phi),
            -pout * cos(theta));

  double energy = sqrt(fPx * fPx + fPy * fPy +
		       fPz * fPz + massMot * massMot);
